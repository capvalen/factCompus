...
while($row=$resultado->fetch_assoc()){ 
	...
	<tr data-id="<?= $row['idProductos']; ?>" data-und="<?= $row['undSunat']; ?>" data-series="<?= $row['series'] ?>">
		...
		<td class='text-capitalize tdProdNombre'><?php if($_COOKIE['ckPower']==1 && $row['idProductos'] <> 1 ){?>
			<button class='btn btn-outline-danger btn-sm border-0 mr-1 btnBorrarProducto'><i class='icofont-close'></i></button>
			<?php } ?> <span><?= $row['prodDescripcion'];?></span></td>
